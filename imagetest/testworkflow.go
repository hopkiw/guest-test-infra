...
func finalizeWorkflows(tests []*TestWorkflow, zone, project string) error {
	...
	for _, ts := range tests {
		...
		ts.destination = fmt.Sprintf("gs://liamh-export/output-from-test-manager/%s/%s/%s", run, ts.Name, ts.ShortImage)
...
func runTestWorkflow(ctx context.Context, test *TestWorkflow) testResult {
	...
	if test.skipped {
		res.skipped = true
		res.err = fmt.Errorf("Test suite was skipped with message: %q", test.SkippedMessage())
		return res
	}
