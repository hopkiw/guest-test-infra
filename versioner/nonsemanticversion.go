...
func (v NonSemanticVer) IncrementVersion() (*NonSemanticVer, error) {
	...
	if strings.Compare(today, v.date) == 0 {
		// i think this implies this component will be running a long
		// time or have some way to instantiate the first time from the
		// known latest build for the day. but where are we going to
		// store that? build code usually has to be stateless.
